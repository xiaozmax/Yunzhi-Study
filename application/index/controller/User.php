...
class User extends Frontend
{
    ...
    public function register()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $authcode = $this->request->post('authcode');//注册码
            $class = Db::table('fa_authcode')->where('code',$authcode)->field('classid')->find();
            $code_in_use = Db::table('fa_authcode')->where('code',$authcode)->field('isusing')->find();;
            if ($class) {
                // 获取 classid 的值
                $class = $class['classid'];
                $code_in_use = $code_in_use['isusing'];
            }
            $mobile = '';
            ...
            if(!$class){
                $this->error(__('Authorization code is invalid'), null, ['token' => $this->request->token()]);
                return ;
            }
            //验证码
            $captchaResult = true;
            ...
            if (!$captchaResult) {
                if($code_in_use == 0){
                    $this->error(__('Authorization code is not enabled'));
                }else{
            ...
            if($code_in_use == 0){
                $this->error(__('Authorization code is not enabled'), null, ['token' => $this->request->token()]);
                return ;
            }
            if ($this->auth->register($username, $password, $email, $mobile, $class)) {
                if($code_in_use == 0){
                    $this->error(__('Authorization code is not enabled'), null, ['token' => $this->request->token()]);
                    return ;
                }else{
