...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        if (!!base64) {
            ...
            var file = dataURLtoFile('data:image/png;base64,' + base64, editor.getOpt('scrawlFieldName') ||'sacle' + '.png');
            let uploadAction = editor.getOpt("uploadService")(this, editor).Upload();
            let Fast = editor.getOpt("uploadService")(this, editor).Fast();
            uploadAction.api.send(file, function (data) {
                editor.execCommand('insertimage', {
                    src: Fast.api.cdnurl(data.url, true)
                });
                dialog.close(false);
            })
